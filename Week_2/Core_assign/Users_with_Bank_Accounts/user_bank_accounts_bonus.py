...
# Sensei Bonus test
michael = User("Michael", "s@s.com")
michael.open_new_account(0.05, 0)
michael.make_deposit(100)
michael.open_new_account(0, 500, "savings").display_user_balance()

rick = User("Rick", "sb@sb.com")
rick.open_new_account(0.045, 300, "savings").display_user_balance()

# SENPAI BONUS TESTS
print("\n"+"*"*30) # prints a line of stars
print("Making Transfers\n")

michael.transfer_money(michael, "checking", 50, "savings").display_user_balance()
michael.transfer_money(rick, "savings", 100, "savings").display_user_balance()
rick.display_user_balance()
rick.transfer_money(michael, "savings", 1000, "checking") # should be insufficient.